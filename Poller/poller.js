...
async function process_target(target, conf, inhObj) {
  ...
  let session = await poller.create_session(
    target,
    conf.options,
    conf.community,
    conf.user
  ...
  let test = await poller.snmp_test(
    target,
    conf.community,
    JSON.parse(JSON.stringify(conf.options)),
    conf.user
  );
  if (test === undefined) {
    const inh =
      "inh_oids" in conf
        ? await poller.get_oids(
            target,
            session,
            conf.inh_oids,
            conf.reportError
    ...
    if ("table" in conf) {
      await Promise.all(
        conf.table.map(async (table) => {
          let flag = true;
          if ("options" in table) {
            if (!("measurement" in table.options)) {
              console.error(
                new func.CustomError(
                  "Config",
                  "No ha declarado measurement dentro de table"
                )
              );
              flag = false;
            }
            ...
            console.error(
              new func.CustomError(
                "Config",
                "No ha declarado options dentro de table"
              )
            );
            flag = false;
          }
          if (flag) {
            const part = await poller.get_table(
              target,
              session,
              table.oids,
              conf.maxRepetitions,
            ...
            for (let k in part) {
              ...
              if (k !== "snmperror") {
                doc =
                  "index" in table.options && table.options.index
                    ? merge(part[k], {
                        tag: { agent_host: target, index: k },
                        measurement_name: table.options.measurement,
                        pollertime: conf.pollertime,
                      })
                    : merge(part[k], {
                        tag: { agent_host: target },
    ...
    if ("oids_get" in conf || "oids_walk" in conf) {
      ...
      if ("oids_walk" in conf) {
        let walk = await poller.get_walk(
          target,
          session,
          conf.oids_walk,
          "array",
...
async function start() {
  try {
    ...
    if (conf.time) conf.pollertime = Date.now() / 1000;
    if (typeof conf.hosts === "string") {
      ...
      func.isObject(conf.hosts) &&
      "type" in conf.hosts &&
      "ipField" in conf.hosts
    ) {
      let data = [];
      if (conf.hosts.type === "mysql") {
        if (!("dbOpt" in conf.hosts && "sql" in conf.hosts))
          throw new func.CustomError(
            "DbConfig",
            "db parameters are incomplete"
          );
            ...
            if (typeof target === "string") {
              ...
              if (ipv4.isValid()) {
                if (func.isObject(doc)) {
                  if ("comField" in conf.hosts && conf.hosts.comField in doc)
                    conf.community = doc[conf.hosts.comField];
        ...
        throw new func.CustomError("DbConfig", "Type of DB is not allowed");
      if (data.length > 0) {
        Promise.all(
          data.map(
            throat(ConLimit, async (doc) => {
              ...
              if (typeof target === "string") {
                ...
                console.error(
                  conf.hosts.ipField + " as target is not a string"
                );
              }
            })
          )
      ...
      throw new func.CustomError(
        "Config",
        "Type of list of hosts are not defined"
      );
  } catch (error) {
    console.error(error);
  }
